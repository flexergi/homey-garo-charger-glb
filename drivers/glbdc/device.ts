...
export class Charger extends Homey.Device {
  ...
  async onInit() {
    ...
    this.registerCapabilityListener("mode", (mode: Mode) =>
  ...
  async setMode(value: Mode) {
    try {
      ...
      const result = await fetch.post(
        `http://${this.address}:8080/${setModeUrl}/${value}`
      );
  ...
  async setCurrentLimit(ampere: number) {
    try {
      ...
      const result = await fetch.post(
        `http://${this.address}:8080/${currentLimitUrl}`,
        data
      );
  ...
  async pollEnergy() {
    try {
      ...
      if (response.stopValue) {
        this.setCapabilityValue("meter_power", response.stopValue).catch(
          this.error
        );
      }
    } catch (e: any) {
  ...
  async getConfig(): Promise<Config> {
    const url = `http://${
      this.address
    }:8080/${configUrl}${new Date().getTime()}`;
  ...
  async pollStatus() {
    try {
      const result: Status = await fetch.json(
        `http://${this.address}:8080/${statusUrl}${new Date().getTime()}`
      );
      ...
      if (currentConnectorValue !== result.connector) {
        this.log(
          `connector changed: current: ${currentConnectorValue} - new: ${result.connector}`
        );
        await this.setCapabilityValue("connector", result.connector).catch(
          this.error
        );
      }
      ...
      if (currentModeValue !== result.mode) {
        this.log(
          `mode changed: current: ${currentModeValue} - new: ${result.mode}`
        );
      ...
      if (currentCurrentLimitValue !== result.currentLimit) {
        this.log(
          `currentLimit changed: current: ${currentCurrentLimitValue} - new: ${result.currentLimit}`
        );
